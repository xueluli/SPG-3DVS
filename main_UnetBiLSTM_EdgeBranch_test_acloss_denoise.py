...
from dataset_LSTM import DatasetFromHdf5_lstm3
from utils import *
from tensorboardX import SummaryWriter
import torchvision.utils as vutils
from metrics import dice_coef, batch_iou, mean_iou, iou_score
from Unet_bidrecionalLSTM_EdgeBranch_test import Unet_BiCLSTM
...
parser.add_argument("--traindata", default="/tilde/xli/3D_Vessel_Segmentation/datasets/Vessel_NN/H5_files/train/train_LSTM_data_new.h5", type=str, help="Training datapath")
...
parser.add_argument("--resume", default="/tilde/xli/3D_Vessel_Segmentation/methods/Ours/model/Unet_BiCLSTM_EdgeBranch_test/model_epoch_101.pth", type=str, help="Path to checkpoint, Default=None")
# parser.add_argument("--resume", default="", type=str, help="Path to checkpoint, Default=None")
#parser.add_argument("--ID", default="_OUTDOOR", type=str, help="dev ID")
parser.add_argument("--ID", default="EdgeBranch_test_acloss_denoise", type=str, help="dev ID")
...
def main():
    ...
    # pdb.set_trace()
    train_set = DatasetFromHdf5_lstm3(file_path=opt.traindata, nt=opt.nt)
    ...
    print("===> Setting GPU")
    model_Feature = torch.nn.DataParallel(model).cuda()
...
# lr policy
    ...
    for iteration, batch in tqdm(enumerate(training_data_loader, 1), total = len(training_data_loader)):
        input, target, edges = Variable(batch[0]).cuda(), Variable(batch[1]).cuda(), Variable(batch[2]).cuda()
        # pdb.set_trace()
        # input_forward = input[:,]
        # input_reverse = 
        output_pre, output, output_lat, output_edg = model(input)
        # pdb.set_trace()
        # output = output1[-1]
        # pdb.set_trace()
        ...
        # loss_ac = active_contour_loss(input[:,:,1,:,:], output_total)
        loss_denoise = torch.sum(torch.abs(output_edg.sigmoid()))/512/512
        loss_ac = active_contour_loss(input[:,1,1,:,:].unsqueeze(1), output)

        # pdb.set_trace()
        loss = loss_seg+loss_edg+0.001*loss_ac+0.01*loss_denoise
