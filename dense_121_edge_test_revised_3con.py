...
# import common
import pdb
# from GridDehazeNet.model import GridDehazeNet
...
class Dense(nn.Module):
    def __init__(self):
        ...
        self.decoder_seg = Dense_decoder()
        self.decoder_edge = Dense_decoder()
        # self.decoder_J = Dense_decoder()
        # self.decoder_w = Dense_decoder()
        # self.decoder_B = Dense_decoder()
        self.convA1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1)
        self.ResA = ResidualBlock(16)
        # self.convA = nn.Conv2d(32, 1, kernel_size=3, stride=1, padding=1)
        # self.sigA = nn.Sigmoid()

        self.convT1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1)
        self.ResT = ResidualBlock(16)
    ...
    def forward(self, x):
        ...
        # print("STARTED ..................")
        # mid = self.dense_block1(x0)
        # # print((x0 != x0).any())
        # # print((self.dense_block1(x0) != self.dense_block1(x0)).any())
        # print((mid != mid).any())
        # # print((self.trans_block2(mid) != self.trans_block2(mid)).any())
        # # print((self.trans_block2(self.dense_block2(x1)) != self.trans_block2(self.dense_block2(x1))).any())
        # # print((x2 != x2).any())
        # #
        # y = self.trans_block1.norm(mid)
        # print((y != y).any())
        # y = self.trans_block1.relu(y)
        # print((y != y).any())
        # y = self.trans_block1.conv(y)
        # print((y != y).any())
        # y = self.trans_block1.pool(y)
        # print((y != y).any())
        #
        # print((x1 != x1).any())
        # # for i in range(6):
        # #     # print(module)
        # #     y = getattr(self.dense_block1, 'denselayer{}'.format(i + 1))(y)
        # #     if (y != y).any():
        # #         print(i)
        # #
        ...
        ######################################
        # pdb.set_trace()
        mask = self.decoder_seg(x, x1, x2, x4)
        # pdb.set_trace()
        # mask = self.convA(self.ResA(self.convA1(mask)))
        mask = self.ResA(self.convA1(mask))
        
        edge = self.decoder_edge(x, x1, x2, x4)
        # pdb.set_trace()
